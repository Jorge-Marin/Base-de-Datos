...
--PROBAR
--[dbo].[agregarSeccionLab] 1001, 'FS1LAB', 'FS-100','11:00', '12:00', 10, 'Lu' ,10, '105', '300'
--SELECT * FROM Registro.smregistro.SeccionLab
--SELECT * FROM Registro.smregistro.Instructor
--DELETE [smregistro].[SeccionLab] WHERE codSeccion = 1001
-- =============================================
...
ALTER PROCEDURE [dbo].[agregarSeccionLab]
		/*
...
-------------------------------
		...
		SET @cantidadClases = (SELECT COUNT([codSeccion]) FROM [smregistro].[SeccionLab] WHERE [codInstructor] = @codInstructor) 
		IF(@cantidadClases = 3 OR @codInstructor NOT IN (SELECT [codInstructor] FROM smregistro.Instructor))
		BEGIN
		...
		*/
		...
		BEGIN
			...
			FROM smregistro.SeccionLab WHERE codSeccion = (SELECT TOP(1) codSeccion FROM #SeccionesImpartiendo)
			...
			WHILE @cantidadDiasImparte > 0
                BEGIN
                    ...
                    IF((SELECT Dia FROM #DiasAsignaturaSeccionNueva WHERE codDia = @codDiaImparte)=(SELECT Dia FROM #DiasSeccionImparte WHERE codDia = @codDiaImparte))
						BEGIN					
						IF(@horaInicio <= @inicioSeccionInstructor   AND @horaFinal>=@FinalSeccionInstructor)
							BEGIN
		...
		INSERT INTO [smregistro].[SeccionLab] VALUES (@codSeccionLab,@codLab, @codAsignatura, @horaInicio , @horaFinal, @cupos,@diasImpartir, @codEdificio,
		@aula , @codInstructor)
		
		UPDATE [smregistro].[Instructor] SET  [cantLaboratorios]= [cantLaboratorios] + 1
