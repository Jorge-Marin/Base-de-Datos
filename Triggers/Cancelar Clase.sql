...
/*
INSERT INTO [smregistro].[CancelacionClase]([codCarrera],[cuentaEstudiante],[codSeccionClase],[codAsignatura],[fecha],[codPeriodo],[descripcion])
VALUES('IS01','20171004244',1200,'FS-100','2020-04-20',1,'')
...
SELECT * FROM Registro.smregistro.CancelacionLabClase
...
INSERT INTO Registro.smregistro.MatriculaClase VALUES ('IS01','20171004244',700,'SC-101','2020-04-20',GETDATE(),1,0,0,'')

INSERT INTO Registro.smregistro.MatriculaClase VALUES ('IS01','20171004244',1200,'FS-100','2020-04-20',GETDATE(),1,0,0,'')
*/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		*/
		...
		SET @codPeriodoActual = (SELECT [codPeriodo] FROM inserted) --sabemos que es el periodo actual por su atributo activo
		...
		*/
		IF(@codPeriodoActual=(SELECT [codPeriodo] FROM [smregistro].[Periodo] WHERE [activo] = 1) AND @fecha = (SELECT [fechaInicio] FROM [smregistro].[Periodo] WHERE [activo] = 1))
		BEGIN
		...
		*/
		...
		PRINT @claseCancelar
		PRINT @fecha
		PRINT @codPeriodoActual
		PRINT 'Cancelado por Coordinación'*/
		END
			ELSE
			BEGIN
		...
		*/
		...
		DELETE [smregistro].[CancelacionClase]
			WHERE @codCarrera = [codCarrera] AND @ctaEstudiante = [cuentaEstudiante] AND @codSeccionClase = [codSeccionClase]
			AND @claseCancelar = [codAsignatura] AND @fecha = [fecha] 
		END
		END
		ELSE
		BEGIN
